Welcome to the codebase for the...
# LEORONIC LOW PERFORMANCE CLUSTER!

Current status: Development.

Leoronic is designed as a highly fault-tolerant cluster management system. If you have some old computers and a router, you can turn them into a cluster using Leoronic.
...
`is_first` runs this task if all older tasks have been completed. Note that
...
`all_with_cmd_done echo *` runs this task if all tasks that match the regex `echo *` are set as
complete.


worker:

`tag has_gpu` runs this task only on a node with the 'has_gpu' tag. Tags are added by
...
`has_file filename.txt` runs this task only on a node that has a given file in its leoronic
